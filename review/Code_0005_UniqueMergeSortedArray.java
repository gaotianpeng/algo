...
public class Code_0005_UniqueMergeSortedArray {
    ...
    public static int[] unqiueMergeSortedArray2(int[] arrA, int[] arrB) {
        int m = arrA != null ? arrA.length : 0;
        int n = arrB != null ? arrB.length : 0;
        ArrayList<Integer> ans = new ArrayList<>();
        ...
        while (i < m && j < n) {
            if (arrA[i] < arrB[j]) {
                if (ans.isEmpty() || ans.get(ans.size() - 1) != arrA[i]) {
                    ans.add(arrA[i]);
                }
                i++;
            } else {
                if (ans.isEmpty() || ans.get(ans.size() - 1) != arrB[j]) {
                    ans.add(arrB[j]);
                }
                j++;
            }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTime; i++) {
            ...
            int[] ans1 = unqiueMergeSortedArray(arrA, arrB);
            int[] ans2 = unqiueMergeSortedArray2(arrA, arrB);
            int[] ans3 = test(arrA, arrB);
